...
it("should return the message if there is no statement", () => {
  expect(format("Quel est ton nom ?", data)).toEqual("Quel est ton nom ?");
});

it("should replace name and age", () => {
  expect(format("Mon nom est {name} et j'ai {age} ans", data)).toEqual(
    "Mon nom est Fabien et j'ai 31 ans"
  );
});

it.skip("should not replace name (escape char)", () => {
  expect(format("Mon nom est '{name}' et j'ai {age} ans", data)).toEqual(
    "Mon nom est '{name}' et j'ai 31 ans"
  );
});

it("should replace unknown fields with an empty string", () => {
  expect(format("Mon nom est {name} et j'ai {unknown} ans", data)).toEqual(
    "Mon nom est Fabien et j'ai  ans"
  );
});
...
it("should replace date", () => {
  expect(
    format(
      "Mon nom est {name} et je suis né le {birthday, date}",
      data,
      "en-US"
    )
...
it("should replace currency", () => {
  expect(
    format("Mon nom est {name} et j'ai gagné {lotery, currency, EUR}", data)
  ).toEqual("Mon nom est Fabien et j'ai gagné €23,292.23");
});

it("should replace currency (test with 0)", () => {
  expect(
    format("Mon nom est {name} et j'ai gagné {loteryZero, currency, EUR}", data)
  ).toEqual("Mon nom est Fabien et j'ai gagné €0.00");
});

it("should replace number", () => {
  expect(
    format("Mon nom est {name} et j'ai gagné {lotery, number}", data)
  ).toEqual("Mon nom est Fabien et j'ai gagné 23,292.23");
});
